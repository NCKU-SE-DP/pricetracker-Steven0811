...
from src.llm_client.llm_client import AnthropicAIClient
...
class TestOpenAIClient(unittest.TestCase):
    ...
    def setUpClass(self):
        if RUN_REAL_API_TESTS:
            self.client = AnthropicAIClient(_api_key=os.getenv("ANTHROPIC_API_KEY"))
        else:
            self.client = AnthropicAIClient(_api_key="fake_api_key")
    ...
    @patch('src.llm_client.llm_client.AnthropicAIClient._generate_text')
